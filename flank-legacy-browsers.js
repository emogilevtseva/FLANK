...
flowScheduler.add(text_6RoutineBegin());
flowScheduler.add(text_6RoutineEachFrame());
flowScheduler.add(text_6RoutineEnd());
...
async function experimentInit() {
  ...
  // Initialize components for Routine "text_6"
  text_6Clock = new util.Clock();
...
function expRoutineBegin(snapshot) {
  return async function () {
    ...
    // keep track of which components have finished
    expComponents = [];
    expComponents.push(text_2);
    expComponents.push(key_resp_2);
...
function fixRoutineEnd(snapshot) {
  return async function () {
    ...
    if (routineForceEnded) {
        routineTimer.reset();} else if (fixMaxDurationReached) {
        fixClock.add(fixMaxDuration);
    } else {
        fixClock.add(0.200000);
    }
    // Routines running outside a loop should always advance the datafile row
    if (currentLoop === psychoJS.experiment) {
      psychoJS.experiment.nextEntry(snapshot);
...
var text_6MaxDurationReached;
var text_6MaxDuration;
var text_6Components;
function text_6RoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'text_6' ---
    t = 0;
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    // keep track of whether this Routine was forcibly ended
    routineForceEnded = false;
    text_6Clock.reset(routineTimer.getTime());
    routineTimer.add(1.000000);
    text_6MaxDurationReached = false;
    // update component parameters for each repeat
    ...
    body: JSON.stringify({ 
    experimentID: 'Um5pgtjKULbI', // * обновить, указав experiment ID из DATAPIPE на шаге 4.3 * 
    filename: filename,  
    data: data, 
    }), 
    ...
    psychoJS.experiment.addData('text_6.started', globalClock.getTime());
    text_6MaxDuration = null
    // keep track of which components have finished
    text_6Components = [];
    text_6Components.push(text_5);
    
    text_6Components.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function text_6RoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'text_6' ---
    // get current time
    t = text_6Clock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
...
function text_6RoutineEnd(snapshot) {
  return async function () {
    //--- Ending Routine 'text_6' ---
    text_6Components.forEach( function(thisComponent) {
      if (typeof thisComponent.setAutoDraw === 'function') {
        thisComponent.setAutoDraw(false);
      }
    });
    psychoJS.experiment.addData('text_6.stopped', globalClock.getTime());
    if (routineForceEnded) {
        routineTimer.reset();} else if (text_6MaxDurationReached) {
        text_6Clock.add(text_6MaxDuration);
    } else {
        text_6Clock.add(1.000000);
    }
    // Routines running outside a loop should always advance the datafile row
    if (currentLoop === psychoJS.experiment) {
      psychoJS.experiment.nextEntry(snapshot);
